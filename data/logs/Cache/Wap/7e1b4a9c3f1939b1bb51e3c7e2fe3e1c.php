...
<div id="mappContainer">
  <div class="inner root" style="height: 468px;">
    <div id="card" class="card" style="background:url('<?php if($card["diybg"] != ''): echo ($card["diybg"]); else: echo ($card["bg"]); endif; ?>') no-repeat 0 0;-webkit-background-size:267px 159px;background-size:267px 159px;">
		  ...
		  <strong class="pdo verify" style="">
				<span style="color:#847d64;text-shadow:0 1px #ebe9e0;">
					...
					<span style="color:<?php echo ($card["numbercolor"]); ?>">待领取</span>
				</span>
		  </strong>
	  </div>
	  <!--会员卡背面-->
	  <div id="card2" class="card" style="background:url('<?php if($card["diybg"] != ''): echo ($card["diybg"]); else: echo ($card["bg"]); endif; ?>') no-repeat 0 0;-webkit-background-size:267px 159px;background-size:267px 159px;display:none">
		  <img src="<?php echo ($card["logo"]); ?>" class="logo"/>
		  <h1 style="color:<?php echo ($card["vipnamecolor"]); ?>;text-shadow:0 1px #e2ded2;"><?php echo ($card["cardname"]); ?></h1>
		  <div style="padding-top:15px;color:#847d64;font-size:12px;margin-left:10px;"><span style="color:<?php echo ($card["vipnamecolor"]); ?>">总店电话：<?php echo ($contact["tel"]); ?></span><p></p>
    ...
    <ul class="round" style="display:block">
     <li style="height:70px;" class="intro">	 
		<div class="footReturn">
			<button id="showcard" class="submit"  onclick="Javascript:window.open('<?php echo U('Userinfo/index',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id']));?>','_parent')"  style="width:100%">点击领取会员卡</button>
     ...
     <ul class="round" style="display:block">
     <li class="intro">
		<div style="width:99%;background:rgb(134, 141, 46);color:white;line-height:30px;padding-left:10px;">商家介绍  - <?php echo ($info["description"]); ?></div>
		<div >
		<?php if($info['logo'] != ''): ?><span style="float:left;padding-righ:10px"><img src="<?php echo ($info["logo"]); ?>"/></span><?php endif; ?>
			<?php echo ($info["info"]); ?>
