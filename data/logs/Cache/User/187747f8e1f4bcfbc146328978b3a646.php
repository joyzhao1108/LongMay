		...
		<?php if(is_array($fun)): $i = 0; $__LIST__ = $fun;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fun): $mod = ($i % 2 );++$i;?><fieldset>
			...
			<ul class="changeapp">
				<?php if(is_array($fun)): $b = 0; $__LIST__ = $fun;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($b % 2 );++$b;?><li>
					<label> <input type="checkbox" <?php if(in_array($list['funname'],$check)): ?>checked="checked"<?php endif; ?> <?php if($list['gid'] > session('gid')): ?>disabled="disabled"<?php endif; ?> onchange="changeapp(this,'<?php echo (session('token')); ?>','<?php echo ($list["id"]); ?>')" value="<?php echo ($list["id"]); ?>"/> <?php echo ($list["name"]); ?></label>
