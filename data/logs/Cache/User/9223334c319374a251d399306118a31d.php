              ...
              <TBODY>
               <TR>
                ...
                <TR>
                  ...
                  <TD>
                   <select name="classid">
                      <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option   value="<?php echo ($vo["id"]); ?>,<?php echo ($vo["name"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>  
                    </select><a href="<?php echo U('Classify/add');?>"  target="ddd" >添加分类</a>
                  </TD>
                </TR>
                  <TH valign="top"><label for="pic">封面图片地址：</label></TH>
                  <TD><input class="text textMiddle inputQ"  name="pic" value="" style="width:480px;"  /><input class="btnFile btnGreen" type=button onclick="window.open('<?php echo U('Upload/index',array('token'=>$token,'n'=>pic));?>')" value="点击上传图片"><br />
                  支持外链和上传到本地或者云相册 
</TD>
                </TR>
               <TR>
